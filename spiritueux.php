...
<html>
	...
	<body>
		<div class="haut">
			...
			<div class="login">
			<div><a class="boutton" href="inscription.php">Inscription</a> <br /></div>
			<br />
			<div><a class="boutton" href="login.php">S'identifier</a></div>
			</div>
		</div>
		<div class="menu">
			<ul>
		...
		<?php
			...
			$tab = mysqli_query($connection, "SELECT * FROM articles WHERE catc='spiritueux'");
			echo "<table>";
			...
			foreach ($tab as $key)
			{
				...
				{
					if ($key2 === 'nom')
						echo "<td>$value <button class='plus' onclick=\"location.href = 'retirepanier.php?nom=$value&red=spiritueux.php'\"> - </button> 

						<button class='plus' onclick=\"location.href = 'ajoutpanier.php?nom=$value&red=spiritueux.php'\"> + </button>";
					if ($key2 === 'prix')
						echo "$value €</td>";
				}
				echo "</tr>";
