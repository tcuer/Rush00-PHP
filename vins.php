...
<html>
	...
	<body>
		...
		<?php
			...
			$tab = mysqli_query($connection, "SELECT * FROM articles WHERE catd='vins'");
			echo "<table>";
			...
			foreach ($tab as $key)
			{
				...
				{
					if ($key2 === 'nom')
						echo "<td>$value <button class='plus' onclick=\"location.href = 'retirepanier.php?nom=$value&red=vins.php'\"> - </button> 

						<button class='plus' onclick=\"location.href = 'ajoutpanier.php?nom=$value&red=vins.php'\"> + </button>";
					if ($key2 === 'prix')
						echo "$value €</td>";
				}
				echo "</tr>";
